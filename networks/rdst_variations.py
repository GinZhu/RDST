...
class CustomSTL(nn.Module):
    r""" Custom Swin Transformer Layer.
    Args:
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_only=False):
        super().__init__()
        self.dim = dim
...
class DualPathwayLayer(nn.Module):
    """ A dual pathway Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if pathway_mode == 'DL':
            no_mlp_in_STL = True
            n_pathway = 2
        elif pathway_mode == 'DH':
            no_mlp_in_STL = True
            n_pathway = 2
        elif pathway_mode == 'QL':
            no_mlp_in_STL = True
            n_pathway = 4
        elif pathway_mode == 'QH':
            no_mlp_in_STL = False
            n_pathway = 4
        else:
            no_mlp_in_STL = False
        ...
        # build blocks
        self.blocks = []
        for i in range(n_pathway):
            pathway = nn.ModuleList([CustomSTL(
                ...
                attention_only=no_mlp_in_STL)
                for i in range(depth)])
            self.blocks.append(pathway)

        # feature fusion
        self.feature_fusion = Mlp(n_pathway * dim, out_features=dim)
